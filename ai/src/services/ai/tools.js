...
export const list_files = tool(
  async ({}, config) => {
    ...
    const response = await axios.get(
      `http://sandbox-service-${config.configurable.sandboxId}:8080/list-files`,
    );
...
export const read_file = tool(
  async ({ files }, config) => {
    ...
    const response = await axios.get(
      `http://sandbox-service-${config.configurable.sandboxId}:8080/read-files?files=${files.join(",")}`,
    );
  ...
  },
  {
    name: "read_file",
    description:
      "Use this tool to read files in the project. Input is an array of file paths. Output is an object with file paths as keys and file content as values.",
    schema: z.object({
      files: z
        .array(z.string())
        .describe(
          'An array of file paths to read eg. ["src/App.jsx", "vite.config.js"]',
        ),
    }),
  },
);
...
export const update_file = tool(
  async ({ files = [] }, config) => {
    ...
    const response = await axios.post(
      `http://sandbox-service-${config.configurable.sandboxId}:8080/update-files`,
      {
        files: payload,
      },
    );
  ...
  },
  {
    name: "update_file",
    description:
      "Use this tool to update/create files in the project. Input is an array of objects with file path and file content to update/create. Output is an object with file paths as keys and file content as values.",
    schema: z.object({
      files: z.array(
        z.object({
          ...
          content: z
            .string()
            .describe("The content of the file to update/create"),
        }),
      ),
    }),
  },
